...
static int flashcache_jobs_init(void)
{
...
static void flashcache_jobs_exit(void)
{
...
static int flashcache_kcached_init(struct cache_c *dmc)
{
 ...
 */
static int flashcache_writeback_md_store(struct cache_c *dmc)
{
...
static int flashcache_writethrough_create(struct cache_c *dmc)
{
...
static int flashcache_writeback_create(struct cache_c *dmc, int force)
{
    struct flash_cacheblock *meta_data_cacheblock, *next_ptr;
    struct flash_superblock *header;
    struct dm_io_region where;
...
static int flashcache_writeback_load(struct cache_c *dmc)
{
    struct flash_cacheblock *meta_data_cacheblock, *next_ptr;
    struct flash_superblock *header;
    struct dm_io_region where;
...
static void flashcache_clean_all_sets(struct work_struct *work)
{
...
static int inline flashcache_get_dev(struct dm_target *ti, char *pth, struct dm_dev **dmd,
...
static void flashcache_dtr_stats_print(struct cache_c *dmc)
{
    ...
    DMINFO("\tdisk reads(%lu), disk writes(%lu) ssd reads(%lu) ssd writes(%lu)\n" \
           ...
           dmc->flashcache_errors.disk_read_errors, dmc->flashcache_errors.disk_write_errors, 
		   dmc->flashcache_errors.ssd_read_errors, dmc->flashcache_errors.ssd_write_errors,
...
void flashcache_status_info(struct cache_c *dmc, status_type_t type,
...
static void flashcache_status_table(struct cache_c *dmc, status_type_t type,
                        char *result, unsigned int maxlen)
{
    u_int64_t  cache_pct, dirty_pct;
    int i;
 ...
 */
void __exit flashcache_exit(void)
{

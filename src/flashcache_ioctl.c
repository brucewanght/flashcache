...
static void flashcache_drop_pids(struct cache_c *dmc, int which_list)
{
    if (which_list == FLASHCACHE_WHITELIST) {
        while (dmc->num_whitelist_pids >= dmc->sysctl_max_pids) {
            VERIFY(dmc->whitelist_head != NULL);
...
static void flashcache_add_pid(struct cache_c *dmc, pid_t pid, int which_list)
{
    struct flashcache_cachectl_pid *new;
    unsigned long flags;
...
static void flashcache_del_pid_locked(struct cache_c *dmc, pid_t pid, int which_list)
{
    struct flashcache_cachectl_pid *node;
    struct flashcache_cachectl_pid **head, **tail;
...
static void flashcache_del_pid(struct cache_c *dmc, pid_t pid, int which_list)
{
    unsigned long flags;
...
static void flashcache_pid_expiry_list_locked(struct cache_c *dmc, int which_list)
{
    struct flashcache_cachectl_pid **head, **tail, *node;
...
int flashcache_ioctl(struct dm_target *ti, unsigned int cmd, unsigned long arg)
{
    struct cache_c *dmc = (struct cache_c *) ti->private;
    struct block_device *bdev = dmc->disk_dev->bdev;
    struct file fake_file = {};

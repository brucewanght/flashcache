...
<<<<<<< HEAD
=======
#define bio_barrier(bio)        ((bio)->bi_opf & REQ_OP_FLUSH)
>>>>>>> modified codes with astyle and to support linux kernel 4.14
/*
 * TODO List :
...
static void flashcache_read_miss(struct cache_c *dmc, struct bio* bio,
				 int index);
=======
#define FLASHCACHE_SW_VERSION "flashcache-3.1.1-lnvm"
char *flashcache_sw_version = FLASHCACHE_SW_VERSION;
...
void flashcache_io_callback(unsigned long error, void *context)
{
    struct kcached_job *job = (struct kcached_job *) context;
    struct cache_c *dmc = job->dmc;
    struct bio *bio;
 ...
 * 4) Free the job.
 */
static void flashcache_do_pending_error(struct kcached_job *job)
{
    struct cache_c *dmc = job->dmc;
    struct cacheblock *cacheblk = &dmc->cache[job->index];
    struct bio *bio = job->bio;
...
static void flashcache_do_pending_noerror(struct kcached_job *job)
{
    struct cache_c *dmc = job->dmc;
    int index = job->index;
    struct pending_job *pending_job, *freelist;
...
static void find_valid_dbn(struct cache_c *dmc, sector_t dbn,
                           int start_index, int *index)
{
    *index = flashcache_hash_lookup(dmc, start_index / dmc->assoc, dbn);
    if (*index == -1)
...
static int find_invalid_dbn(struct cache_c *dmc, int set)
{
    int index = flashcache_invalid_get(dmc, set);
 ...
 * dbn is the starting sector, io_size is the number of sectors.
 */
static int flashcache_lookup(struct cache_c *dmc, struct bio *bio, int *index)
{
    sector_t dbn = bio->bi_sector;
#if DMC_DEBUG
    int io_size = to_sector(bio->bi_size);
 ...
 * Cache Metadata Update functions
 */
void flashcache_md_write_callback(unsigned long error, void *context)
{
    struct kcached_job *job = (struct kcached_job *)context;
...
static int flashcache_alloc_md_sector(struct kcached_job *job)
{
    struct page *page = NULL;
    struct cache_c *dmc = job->dmc;
    unsigned long addr = 0;
...
static void flashcache_free_md_sector(struct kcached_job *job)
{
    if (job->pl_base[0].page != NULL)
        __free_pages(job->pl_base[0].page, get_order(MD_BLOCK_BYTES(job->dmc)));
    job->pl_base[0].page = NULL;
...
static void flashcache_kcopyd_callback(int read_err, unsigned int write_err, void *context)
{
    struct kcached_job *job = (struct kcached_job *)context;
    struct cache_c *dmc = job->dmc;
    int index = job->index;
...
=======
static inline int flashcache_can_clean(struct cache_c *dmc,
                                       struct cache_set *cache_set,
                                       int nr_writes)
{
    return ((cache_set->clean_inprog + nr_writes) < dmc->max_clean_ios_set &&
...
static void flashcache_read_hit(struct cache_c *dmc, struct bio* bio, int index)
{
    struct cacheblock *cacheblk;
    struct pending_job *pjob;
    int set = index / dmc->assoc;
 ...
 * until the underlying IO is finished, and then proceed with the invalidation.
 */
static int flashcache_inval_block_set(struct cache_c *dmc, int set, struct bio *bio, int rw,
                                      struct pending_job *pjob)
{
    sector_t io_start = bio->bi_sector;
    sector_t io_end = bio->bi_sector + (to_sector(bio->bi_size) - 1);
...
static int flashcache_inval_block_set_v3(struct cache_c *dmc, int set, struct bio *bio,
        struct pending_job *pjob)
{
    int index;
    struct cacheblock *cacheblk;
...
static void flashcache_write_miss(struct cache_c *dmc, struct bio *bio, int index)
{
    struct cacheblock *cacheblk;
    struct kcached_job *job;
    int queued;
...
static void flashcache_write_hit(struct cache_c *dmc, struct bio *bio, int index)
{
    struct cacheblock *cacheblk;
    struct pending_job *pjob;
    struct kcached_job *job;
...
static void flashcache_do_block_checks(struct cache_c *dmc, struct bio *bio)
{
    sector_t mask;
    sector_t io_start;
    sector_t io_end;
...
/* Block sync support functions */
static void flashcache_kcopyd_callback_sync(int read_err, unsigned int write_err, void *context)
{
    struct kcached_job *job = (struct kcached_job *)context;
    struct cache_c *dmc = job->dmc;
    int index = job->index;
...
static void flashcache_uncached_io_callback(unsigned long error, void *context)
{
    struct kcached_job *job = (struct kcached_job *) context;

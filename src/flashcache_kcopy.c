...
static struct flashcache_copy_job* alloc_flashcache_copy_job(struct cache_c *dmc)
{
    unsigned long flags;
    struct flashcache_copy_job *job;
 ...
 * this copy, we added back the copy_job to the preallocated pool.
 */
static void free_flashcache_copy_job(struct cache_c *dmc, struct flashcache_copy_job *job)
{
    unsigned long flags;
    int i;
...
struct flashcache_copy_job* new_flashcache_copy_job(struct cache_c *dmc,
                        int nr_writes,
                        struct dbn_index_pair *writes_list)
{
    struct flashcache_copy_job *job;
...
static int dm_io_async_pagelist_IO(struct flashcache_copy_job *job,
                        unsigned int num_regions,
                        struct dm_io_region *where,
                        io_notify_fn fn,
                        int rw,
...
void flashcache_handle_read_write_error(struct flashcache_copy_job *job)
{
    struct kcached_job *io_error_job;
    struct cache_c *dmc = job->dmc;
    int set;
...
void flashcache_copy_data_write_callback(unsigned long error, void *context)
{
    struct flashcache_copy_job *job =
        (struct flashcache_copy_job *)context;
 ...
 * When all of the reads complete, we kick off the write
 */
void flashcache_copy_data_read_callback(unsigned long error, void *context)
{
    struct flashcache_copy_job *job =
        (struct flashcache_copy_job *)context;
    unsigned long flags;
...
static void flashcache_verify_chain(struct cache_c *dmc,
                        int nr_writes,
                        struct dbn_index_pair *writes_list)
{
    int i;
...
int flashcache_copy_data_one_chain(struct cache_c *dmc,
                               struct cache_set *cache_set,
                               int nr_writes,
                               struct dbn_index_pair *writes_list)
{
...
static void flashcache_verify_sorted(struct cache_c *dmc,
                         int nr_writes,
                         struct dbn_index_pair *writes_list)
{
    int i;
...
void flashcache_copy_data(struct cache_c *dmc,
                     struct cache_set *cache_set,
                     int nr_writes,
                     struct dbn_index_pair *writes_list)
{

		       ...
		       sector_t dbn)
{
	...
	return -1;
=======
int flashcache_cleaning_read_empty(void)
{
    return list_empty(&_cleaning_read_complete_jobs);
}

int flashcache_cleaning_write_empty(void)
{
    return list_empty(&_cleaning_write_complete_jobs);
}
...
int flashcache_uncached_io_complete_empty(void)
{
    return list_empty(&_uncached_io_complete_jobs);
}
...
static inline u_int16_t* flashcache_get_hash_bucket(struct cache_c *dmc, struct cache_set *cache_set, sector_t dbn)
{
    unsigned int hash = jhash_1word(dbn, 0xfeed);
...
/* Must return -1 if not found ! */
int flashcache_hash_lookup(struct cache_c *dmc,
                       int set,
                       sector_t dbn)
{
    struct cache_set *cache_set = &dmc->cache_sets[set];
		       ...
		       int index)
{
	...
	*hash_bucket = set_ix;
=======
void flashcache_hash_insert(struct cache_c *dmc,
                       int index)
{
    struct cache_set *cache_set = &dmc->cache_sets[index / dmc->assoc];
    struct cacheblock *cacheblk;
...
void push_cleaning(struct list_head *jobs, struct flashcache_copy_job *job)
{
    unsigned long flags;
...
struct flashcache_copy_job* pop_cleaning(struct list_head *jobs)
{
    struct flashcache_copy_job *job = NULL;
...
static void process_clean_jobs(struct list_head *jobs,
                   void (*fn) (struct flashcache_copy_job *))
{
    struct flashcache_copy_job *job;
    int done = 0;
...
struct kcached_job* new_kcached_job(struct cache_c *dmc, struct bio* bio, int index)
{
    struct kcached_job *job;
...
static void flashcache_record_latency(struct cache_c *dmc, struct timeval *start_tv)
{
    struct timeval latency;
    int64_t us;
...
static int cmp_dbn(const void *a, const void *b)
{
    if (((struct dbn_index_pair *)a)->dbn < ((struct dbn_index_pair *)b)->dbn)
        return -1;
    else
...
static void swap_dbn_index_pair(void *a, void *b, int size)
{
    struct dbn_index_pair temp;
...
flashcache_update_sync_progress(struct cache_c *dmc)
{
	...
	printk(KERN_INFO "\r");
=======
void flashcache_merge_writes(struct cache_c *dmc, struct dbn_index_pair *writes_list,
                        struct dbn_index_pair *set_dirty_list,
                        int *nr_writes, int set)
{
    int dirty_blocks_in = *nr_writes;
...
int flashcache_dm_io_async_vm(struct cache_c *dmc, unsigned int num_regions,
                          struct dm_io_region *where,
                          int rw,
                          void *data, io_notify_fn fn, void *context)
{
...
int flashcache_dm_io_sync_vm(struct cache_c *dmc, struct dm_io_region *where, int rw, void *data)
{
    unsigned long error_bits = 0;
    int error;
    struct dm_io_request io_req = {
...
void flashcache_diskclean_free(struct cache_c *dmc, struct dbn_index_pair *buf1, struct dbn_index_pair *buf2)
{
    unsigned long flags;
    struct diskclean_buf_ *diskclean_buf;

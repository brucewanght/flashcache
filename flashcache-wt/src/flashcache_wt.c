...
static u_int64_t flashcache_wt_compute_checksum(struct bio *bio)
{
    int i;
    u_int64_t sum = 0, *idx;
    int cnt;
...
static void flashcache_wt_store_checksum(struct kcached_job *job)
{
    u_int64_t sum;
    unsigned long flags;
...
static int flashcache_wt_validate_checksum(struct kcached_job *job)
{
    u_int64_t sum;
    int retval;
    unsigned long flags;
 ...
 */
void flashcache_wt_io_callback(unsigned long error, void *context)
{
    struct kcached_job *job = (struct kcached_job *) context;
    struct cache_c *dmc = job->dmc;
    struct bio *bio;
...
int flashcache_wt_do_complete(struct kcached_job *job)
{
    struct bio *bio = job->bio;
    struct cache_c *dmc = job->dmc;
    unsigned long flags;
...
static int cache_invalidate_block_set(struct cache_c *dmc, int set, sector_t io_start, sector_t io_end,
 ...
 */
int flashcache_wt_map(struct dm_target *ti, struct bio *bio,
...
static void flashcache_wt_uncached_io_callback(unsigned long error, void *context)
{
    struct kcached_job *job = (struct kcached_job *) context;
    struct cache_c *dmc = job->dmc;
    unsigned long flags;
...
static void flashcache_wt_start_uncached_io(struct cache_c *dmc, struct bio *bio)
{
...
static int inline flashcache_wt_get_dev(struct dm_target *ti, char *pth, struct dm_dev **dmd,
                      char *dmc_dname, sector_t tilen)
{
    int rc = dm_get_device(ti, pth,
...
static void flashcache_wt_status_info(struct cache_c *dmc, status_type_t type,
...
static void flashcache_wt_status_table(struct cache_c *dmc, status_type_t type,
                           char *result, unsigned int maxlen)
{
    int cache_pct;
    int i;
...
static int flashcache_wt_version_show(struct seq_file *seq, void *v)
{
    seq_printf(seq, "Flashcache_wt Version : %s\n", flashcache_wt_sw_version);
    return 0;
}

static int flashcache_wt_version_open(struct inode *inode, struct file *file)
{
 ...
 */
int __init flashcache_wt_init(void)
{
    int r;
